/**
 * The whole streaming tool loop, with nothing else in the file -- TypeScript/Node.
 *
 * No tool code, no `tools`, no formatting: render a prompt, stream it, print each
 * event as it lands. This works because every tool bound to the prompt's `production`
 * alias has an `http` executor -- the platform runs those, so there is nothing for
 * this script to supply.
 *
 * If a bound tool's version has a `client` executor instead, its code lives in your
 * process and the platform cannot reach it. The loop then stops before it calls the
 * model and says so, naming the tool. That is the only case that needs `clientTools`;
 * see stream_prompt_tools.mjs, which handles both.
 *
 * Requires:
 *   npm install @acruxcoreai/sdk
 *
 * Env:
 *   ACRUXCORE_API_KEY   -- required
 *   ACRUXCORE_BASE_URL  -- required, no default (e.g. http://localhost:3001/api/v1)
 *
 * Run:
 *   node stream_minimal.mjs
 */
import { acruxcore } from '@acruxcoreai/sdk';

const hub = new acruxcore();

const rendered = await hub.prompts.render('weather-brief', 'production', { city: 'Lisbon' });

const stream = await hub.gateway.runPromptWithTools(rendered, { stream: true });
for await (const event of stream) {
  console.log(event);
}

// The loop reports its trace in the background; wait for that write so the trace is
// readable the moment this script exits.
await hub.gateway.close();
