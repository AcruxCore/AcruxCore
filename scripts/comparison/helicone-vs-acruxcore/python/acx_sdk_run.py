...
Contrast with `hl_trace_run.py`: Helicone's Playground templating is flat
`{{ hc:var:type }}` substitution, so that script inlines the system/user text
too. Here the prompt lives on the platform — `hub.prompts.render()` fetches
and fills the `vip-support-triage@production` template, `hub.gateway.chat()`
sends it through the gateway, and the trace (with prompt-version lineage
attached) is written server-side with no tracing code in this file at all.

Run:
  export ACRUXCORE_API_KEY=acx_sk_...
  export ACRUXCORE_BASE_URL=http://localhost:3001/api/v1
  python scripts/comparison/helicone-vs-acruxcore/python/acx_sdk_run.py
