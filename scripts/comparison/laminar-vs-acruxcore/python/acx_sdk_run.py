...
Contrast with `lm_trace_run.py`: Laminar has no prompt registry at all, so that
script inlines the flattened system/user text and wraps a normal OpenAI call with
Laminar's OpenTelemetry instrumentation. Here the prompt lives on the platform — `hub.prompts.render()`
fetches and fills the `vip-support-triage@production` template,
`hub.gateway.chat()` sends it through the gateway, and the trace (with
prompt-version lineage attached) is written server-side with no tracing code
in this file at all.

Run:
  export ACRUXCORE_API_KEY=acx_sk_...
  export ACRUXCORE_BASE_URL=http://localhost:3001/api/v1
  python scripts/comparison/laminar-vs-acruxcore/python/acx_sdk_run.py
