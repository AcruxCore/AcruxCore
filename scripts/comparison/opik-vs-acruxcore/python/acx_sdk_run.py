...
Contrast with `op_trace_run.py`: Opik has no runtime "render this stored prompt"
call, so that script inlines the flattened system/user text and wraps a normal
OpenAI call. Here the prompt lives on the platform — `hub.prompts.render()`
...
Run:
  ...
  python scripts/comparison/opik-vs-acruxcore/python/acx_sdk_run.py
