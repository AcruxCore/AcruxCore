...
![Connecting get_weather to support-reply](/img/tutorials/product-tour/03-attach-tool.png)

:::tip[No SDK? Connect a tool from the dashboard or curl]
...
Everything in this tour was a change to the workspace: a prompt committed, a
tool attached, a key created, an alias promoted. **Team → Audit trail** is the
record of all of them, newest first, with the person behind each one:

![The Audit trail page reading 1,005 events recorded, with area, event and actor filters above a table of API key and prompt events, each naming the actor](/img/tutorials/product-tour/08-audit-trail.png)
...
:::note[Traces and audit events are different records]
...
[Read the team audit trail](../guides/read-the-team-audit-trail).
:::
