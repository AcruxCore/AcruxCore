...
| `maxRetries` | `number` | `1` | Retries **this SDK's own request to AcruxCore** on a transient failure (2 total attempts). It does not change how many times a provider is called — that is the per-call [`gateway.maxRetries`](#gatewaychatoptions). |
...
| You have | Call |
|---|---|
| Messages you wrote, no tools | [`gateway.chat`](#gatewaychatoptions) |
| Messages you wrote, and the tools named on the call | [`gateway.runToolLoop`](#gatewayruntoolloopoptions) |
| A prompt from the catalog | [`prompts.render`](#promptsrendername-alias-variables), then [`gateway.runPromptWithTools`](#gatewayrunpromptwithtoolsrendered-options) |

`chat()` takes `tools` too, but only *offers* them: it returns with
`finishReason: 'tool_calls'` and the request on `message.tool_calls`, and running it is
yours to do. It also rejects a tool declared with `acrux.tool` — that shape belongs to
`runToolLoop`, and the error says so.
...
| The catalog, `http` executor | AcruxCore | Nothing — the prompt's binding is enough, or `toolRefs: [{ name }]` |
| The catalog, `client` executor | Your process | `clientTools: { get_weather: getWeather }` |
| Nowhere — inline for this one call | Your process | `toolDefs: [...]` and `dispatch` |
...
| `gateway` | `{ maxRetries?: number; fallback?: boolean }` | no | Per-call gateway controls: how many times the **provider** may be retried (0–5), and whether a failure may fall back to the next model in the chain. A different layer from the client's `maxRetries`, which only retries this SDK's request to AcruxCore. Ignored on a BYO call. |
...
| `provider` | [`ProviderConfig`](#byo-provider) | no | Per-call BYO override. |
| `gateway` | `{ maxRetries?: number; fallback?: boolean }` | no | Per-call gateway controls: how many times the **provider** may be retried (0–5), and whether a failure may fall back to the next model in the chain. A different layer from the client's `maxRetries`, which only retries this SDK's request to AcruxCore. Ignored on a BYO call, and applied to every round of the loop. |
...
`wait: true` (the default) awaits the POST: errors throw at the call site and
`traceId` is the server's. `{ wait: false }` buffers the trace and returns
...
gateway's spans, which share the buffer — and `gateway.close()` flushes too.

## `traces.flush()`

Wait for every trace buffered by `ingest(…, { wait: false })` (and by the
gateway's own span reporting) to be sent. Resolves once the queue is empty.
