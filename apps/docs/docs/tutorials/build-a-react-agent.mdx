...
Each binding lands as the prompt's default, so every alias of the prompt — including
the two the first commit just created — calls both tools. See
[Connect a tool to a prompt](../guides/connect-a-tool-to-a-prompt) for giving one
alias a different build.
...
Full script: [`setup_prompt.py`](https://github.com/AcruxCore/AcruxCore/blob/main/scripts/tutorials/react-agent/python/setup_prompt.py).
