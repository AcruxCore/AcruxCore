...
description: A finance research agent that reasons step by step and calls two real tools, driven by a manual tool-calling loop straight against your own model provider — no gateway.
sidebar_position: 5
...
[`react_agent.ipynb`](https://github.com/AcruxCore/AcruxCore/blob/main/scripts/tutorials/react-agent/notebook/react_agent.ipynb)
...
2. **Complete** — `POST $PROVIDER_BASE_URL/chat/completions`, straight to your provider, with your own key. Never through AcruxCore.
3. **Report the `llm` span yourself** — because no gateway saw that call, nothing records it unless you do. `POST /traces` with `kind: "llm"`, the model, `provider` set to the host from your base URL (derive it rather than typing it, or the span will name a provider you stopped using), and the token usage the provider returned.
...
Run standalone first, to see what the real tool returns before wiring it into a loop. The
same two calls as the curl tab, in the same order:
