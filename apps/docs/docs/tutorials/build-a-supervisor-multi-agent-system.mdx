...
[`supervisor_flow.ipynb`](https://github.com/AcruxCore/AcruxCore/blob/main/scripts/tutorials/build-a-supervisor-multi-agent-system/notebook/supervisor_flow.ipynb)
...
[Build a configurable ReAct agent](./build-a-configurable-react-agent) uses.
...
a dashboard/API version" behavior the SDK documents. Open **Tools → finance_research**
and both versions are visible, `v1` tagged `api` and `v2` tagged `code`:

![finance_research tool detail page showing v2 from code carrying the production chip, above v1 from the API](/img/tutorials/build-a-supervisor-multi-agent-system/02-tool-versions.png)

All four now appear in the catalog:

![The catalog filtered to research, listing finance_research, basic_research and advanced_research, each marked Runs in your code](/img/tutorials/build-a-supervisor-multi-agent-system/03-tools-list.png)
