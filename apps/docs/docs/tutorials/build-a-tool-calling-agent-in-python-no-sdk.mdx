...
async with AcruxCore() as hub:
    ...
    await hub.prompts.commitVersion(
        prompt.id,
        messages=[
            {"role": "system", "content": "You are a weather assistant. Use the get_weather tool to look up conditions before answering. Never guess."},
            {"role": "user", "content": "What is the weather in {{ city }} right now?"},
...
Setting `stream: true` and reading the SSE frames is the one Acrux Core-specific part:
...
async with AcruxCore() as hub:
    credential = await hub.gateway.createConnection(
        provider="openai_compatible",
        label="OpenRouter",
        apiKey="<your openrouter key>",
        config={"base_url": "https://openrouter.ai/api/v1"},
    )
    model = await hub.gateway.createModel(
        publicName="gpt-4o-mini",
        upstreamModel="openai/gpt-4o-mini",
...
const hub = acruxcore({ apiKey: process.env.ACRUXCORE_API_KEY });
const credential = await hub.gateway.createConnection({
  provider: 'openai_compatible',
  label: 'OpenRouter',
  apiKey: '<your openrouter key>',
  config: { base_url: 'https://openrouter.ai/api/v1' },
});
const model = await hub.gateway.createModel({
  publicName: 'gpt-4o-mini',
  upstreamModel: 'openai/gpt-4o-mini',
  credentialId: credential.id,
});
