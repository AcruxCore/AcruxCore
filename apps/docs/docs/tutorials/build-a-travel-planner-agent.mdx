...
### A shell, plus versions

A tool is stored as a **shell** plus a list of immutable **versions**. The shell is only an identity: a name, and a description used as its label in the catalog. Everything the model reads, and everything the executor needs, lives on a version — so a shell on its own resolves to nothing and cannot be called.

The dashboard collects both at once. **Tools → New tool** asks for the name and then for the whole first version, and one click writes them together:

![New tool dialog asking for the name, the model-facing description, the parameters and the executor in one form](/img/tutorials/build-a-travel-planner-agent/09-new-tool-dialog.png)

From code it is two calls, because the shell has to exist before a version can be committed onto it:

| Step | What it stores | From code |
|---|---|---|
| **1. Create the shell** | name, catalog label | `POST /tools` — `tools.create()` |
| **2. Commit version 1** | description, parameters schema, executor | `POST /tools/:id/versions` — `commit_version()` / `commitVersion()` |
...
- **Two fields are called `description`** — one on the shell, one on the version — and only one of them normally reaches the model: the version's. The shell's is a fallback, used when the version has none. The New tool dialog asks only for the version's, which is the one that matters; the shell's is set later under **Settings** if you want a separate label for your team.
...
This is the catalog once all three exist. Each row says which version `production` points at and who runs the call, so a tool that cannot be called yet is visible at a glance:

![The catalog listing search_flights marked Runs in your code, and convert_currency and get_city_weather marked Runs on AcruxCore, each row showing where production points](/img/tutorials/build-a-travel-planner-agent/01-tool-catalog.png)
...
Go to **Tools → New tool** and fill in four fields:

- **Name** — `get_city_weather`.
- **Description** — "Get the current weather and a three-day forecast for one city. The forecast only reaches three days ahead, so it cannot answer a question about a season or a typical climate."
...
- **Executor** — `HTTP — AcruxCore calls a URL for you`, method `GET`, URL `https://wttr.in/{{arg.city}}`, one query param `format` = `j1`, and the function above pasted into **Response transform**.

Then click **Create tool**. That writes the tool and version 1 together, and because it is the first version, `production` and `staging` are created for you, both pointing at it.
...
![New tool dialog for the weather tool: description, changelog, one city parameter row with reject-unknown ticked, and the HTTP executor below](/img/tutorials/build-a-travel-planner-agent/02-tool-description.png)
...
`headers` is left out of the executor because it defaults to an empty list.
...
The same dialog, with two differences worth understanding. This tool takes three arguments instead of one, and they go into **query params** rather than the URL path. It also needs no `responseTransform`, because [Frankfurter](https://frankfurter.dev) already answers with a small object.
...
Go to **Tools → New tool**, name it `convert_currency`, then:

- **Description** — "Convert an amount of money from one currency to another at today's reference exchange rate."
...
- **Executor** — `HTTP — AcruxCore calls a URL for you`, method `GET`, URL `https://api.frankfurter.dev/v1/latest`, and three query params: `amount` = `{{arg.amount}}`, `base` = `{{arg.from}}`, `symbols` = `{{arg.to}}`.
...
![New tool dialog for the currency tool showing three query param rows that map amount, base and symbols to templated model arguments, with both transform fields empty](/img/tutorials/build-a-travel-planner-agent/11-currency-query-params.png)
...
Go to **Tools → New tool**, name it `search_flights`, then:
...
- **Executor** — choose `Client — your own code runs it`. The URL, header, query and transform fields disappear, because there is nothing for the platform to call.

Then **Create tool**.

![New tool dialog for the flight tool with the executor dropdown set to Client, your own code runs it, and no URL fields shown](/img/tutorials/build-a-travel-planner-agent/10-client-executor.png)
...
**Step 2 — version 1.** In the editor, keep one message, set its role to **SYSTEM** and paste the text above. Set the **default model** to `mistral-small`, then **Commit version**. A prompt is still two steps, unlike a tool — and as with tools, the first version becomes both `production` and `staging`.

The editor highlights `{{ today }}` as a variable, and the header shows which aliases point at this version:

![The travel planner prompt in the editor: one SYSTEM message with the today variable highlighted, default model mistral-small, production and staging both at v1](/img/tutorials/build-a-travel-planner-agent/04-system-prompt.png)
...
![The prompt's Tools tab showing convert_currency, get_city_weather and search_flights each following production in the default column, with two aliases inheriting](/img/tutorials/build-a-travel-planner-agent/05-three-tool-bindings.png)
...
- [Create a tool](../guides/create-a-tool) — the tool catalog on its own, in more depth than this tutorial needs.
