...
![Tools tab with web_research connected in the default column](/img/tutorials/build-a-configurable-react-agent/05-tools-tab.png)
...
curl has no `run_prompt_with_tools()`/`runPromptWithTools()` equivalent — see [web_research_agent.sh on GitHub](https://github.com/AcruxCore/AcruxCore/blob/main/scripts/tutorials/configurable-react-agent/bash/web_research_agent.sh) for the complete source.
...
`client_tools`, keyed by the catalog tool name — and that function calls
`langchain_community`'s real `TavilySearchResults` class directly, exactly the two
configurations the source defines. The definition itself stays in the catalog, so the
binding this alias resolved is what the model is offered.

The full script uses `run_prompt_with_tools()` to drive the loop, with the `client_tools` entry calling `langchain_community`'s `TavilySearchResults` directly — see [run_agent.py on GitHub](https://github.com/AcruxCore/AcruxCore/blob/main/scripts/tutorials/configurable-react-agent/python/run_agent.py) for the complete source.
...
The full script uses `runPromptWithTools()` to drive the loop, with the `clientTools` entry calling `@langchain/tavily`'s `TavilySearch` directly — see [run_agent.mjs on GitHub](https://github.com/AcruxCore/AcruxCore/blob/main/scripts/tutorials/configurable-react-agent/typescript/run_agent.mjs) for the complete source.
