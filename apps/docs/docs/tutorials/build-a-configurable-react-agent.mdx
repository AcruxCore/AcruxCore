...
[`web_research_agent.ipynb`](https://github.com/AcruxCore/AcruxCore/blob/main/scripts/tutorials/configurable-react-agent/notebook/web_research_agent.ipynb)
...
A [Tavily](https://tavily.com/) API key (the free tier is enough). The Python tab uses
...
Open **Tools** and click **New tool**. Name it `web_research`, give it one
required `query` parameter of type `string`, and leave the executor on
**Client — your own code runs it**: the platform stores the schema, your own
code runs the real Tavily call. The schema is deliberately thin — the model only
ever supplies a `query`; which depth that query runs at is decided by which
alias is active, not by the model.

![New tool dialog with the name web_research, a description about searching the live web, a required query string parameter, and the executor set to Client](/img/tutorials/build-a-configurable-react-agent/01-new-tool.png)

Click **Create tool**. `web_research` now exists with one parameter and no
...
![Editor tab now showing default model claude-haiku and a system message describing the thorough, deep-search persona](/img/tutorials/build-a-configurable-react-agent/06-deep-editor.png)
...
![Versions tab listing v2 on claude-haiku and v1 on gemini-flash, with production and staging both still on v1](/img/tutorials/build-a-configurable-react-agent/07-versions-tab.png)
...
The full script uses `run_prompt_with_tools()` to drive the loop, with the `client_tools` entry calling `tavily-python`'s `AsyncTavilyClient` — see [run_agent.py on GitHub](https://github.com/AcruxCore/AcruxCore/blob/main/scripts/tutorials/configurable-react-agent/python/run_agent.py) for the complete source.
...
![Trace tree for the Node deep run showing an LLM span, a web_research tool span, and a final LLM span, all on claude-haiku](/img/tutorials/build-a-configurable-react-agent/13-trace-tree.png)
