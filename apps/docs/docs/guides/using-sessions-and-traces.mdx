...
```typescript
for (const [name, variables] of prompts) {
  const { messages, model, versionId, tools } = await hub.prompts.render(name, 'production', variables);
...
Feedback also aggregates: an average rating grouped by model or prompt
version, and a team-wide raw feed across every trace.
...
Real output for the two session reads above, from a session with three
tour-planner traces already in it:
...
- Now that you have traces and sessions, see the aggregate picture:
  [View trace analytics](./view-trace-analytics) — `group_by=session`
  reads the same session ids this guide sets up.
