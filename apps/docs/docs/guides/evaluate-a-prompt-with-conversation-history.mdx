...
<TabItem value="node" label="Node (fetch)">
...
experiment/run calls themselves — is REST-only today: this domain isn't wrapped by
either published SDK yet, so the examples below call the API directly instead of
going through `@acruxcoreai/sdk` or `acruxcore`.
