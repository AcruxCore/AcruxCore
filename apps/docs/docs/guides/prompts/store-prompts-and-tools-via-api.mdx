...
slug: /guides/store-prompts-and-tools-via-api
---
...
Steps 2 and 3 have a UI equivalent: **New tool** writes a tool and its first
version from one form, **New prompt** creates a prompt shell whose detail page
commits versions, and the prompt's **Tools** tab connects tools. The screenshots in the next step are that same dashboard — the
...
The tools themselves live in **Tools**:
