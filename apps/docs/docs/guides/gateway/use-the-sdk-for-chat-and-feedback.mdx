...
slug: /guides/use-the-sdk-for-chat-and-feedback
---
...
console.log(result.gateway);      // { requestId, provider, model, costUsd, cache, traceId, spanRef }
```
...
`hub.gateway.chat()` doesn't write a second one. You don't have to go looking for it:
the gateway metadata hands you `traceId` (`trace_id` in Python) for
[the trace it produced](./trace-an-llm-call), and `spanRef` for the `llm` span inside
it. That trace id is what you pass to `submitFeedback` and what an evaluation dataset
is later built from.
...
If your prompt has [tools attached](./connect-a-tool-to-a-prompt), rendering it
...
[Create a tool](./create-a-tool) — and `toolDefs` plus `dispatch`
...
[Create a tool](./create-a-tool) — and `tool_defs` plus
