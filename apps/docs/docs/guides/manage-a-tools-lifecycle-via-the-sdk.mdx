...
`{{arg.order_id}}` in the query value is where the argument goes. The platform
replaces it at call time with the `order_id` the caller sent. The same
`{{arg.NAME}}` form works in a header value and in the URL.
...
curl -X POST "$ACRUXCORE_BASE_URL/tools/41fd848e-ad01-43bb-bb3c-44869616d321/versions" \
  ...
  -d '{
    ...
    "executor": {"type":"http","url":"https://httpbin.org/get","method":"GET","headers":[],"query":[{"name":"order_id","value":"{{arg.order_id}}"}]}
  }'
```
...
```json
{
  ...
  "executor": {
    ...
    "type": "http",
    "query": [{ "name": "order_id", "value": "{{arg.order_id}}" }],
    "method": "GET",
    "headers": [],
    "argMapping": []
  },
...
```text
{"url":"https://httpbin.org/get","type":"http","query":[{"name":"order_id","value":"{{arg.order_id}}"}],"method":"GET","headers":[],"argMapping":[]}
```
