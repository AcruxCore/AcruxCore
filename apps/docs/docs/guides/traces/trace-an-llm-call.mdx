...
print("reported trace", result.trace_id)
```

`traces.ingest()` takes the same dict shape as the raw endpoint — its keys stay
camelCase because they're sent to the API verbatim, unlike the rest of the
Python client's snake_case options.
