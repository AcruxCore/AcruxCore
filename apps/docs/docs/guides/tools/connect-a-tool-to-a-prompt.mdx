...
sidebar_position: 2
slug: /guides/connect-a-tool-to-a-prompt
...
This picks up where [Create a tool](./create-a-tool) leaves off. That guide puts
the tool in the catalog; this one decides which build of it each of your prompt's
aliases calls. Running it is the step after, in
...
| Cell reads | Meaning |
|---|---|
| `production → v3` | this alias follows the tool's `production` alias, which currently points at v3 |
| `pinned to v2` | this alias is locked to tool version 2 — no alias move reaches it |
| `same as default` | no binding of its own; the default column applies |
| `not used here` | this alias deliberately does not call the tool, even though the default does |
| `not connected` | the prompt does not call this tool at all |

`not used here` exists because an empty cell already means *same as default*. It is
how you keep a billing call out of `staging` while `production` keeps it.
...
To change what the default follows, click the cell. The dropdown groups its options: every
alias the tool has under **Follow an alias — moves when it is promoted**, every
version number under **Pin a version — never moves**, and, on an alias column,
**Not for this alias** under **Stop calling it**. A pin is the stronger promise: it never moves, so a
...
that alias gets a column of its own. Its cells start on `same as default`; set one and
...
running the tested one. Or pick **Not for this alias** under **Stop calling it**, for a tool a test run must
never call.
...
Two ways back to the default: pick **Go back to the default** in one cell, or use the
...
![The tool's Audit tab listing prompt-binding changes and version commits, the commits tagged "code sync"](/img/tutorials/connect-a-tool-to-a-prompt/03-tool-audit-tab.png)
...
- [Call a prompt's tools from the SDK](./call-a-prompts-tools-from-the-sdk) — run
  the tool you just bound, streaming or not.
- [Create a tool](./create-a-tool) — where the tool this guide binds came from.
- [Version and track a tool](./version-and-track-a-tool) — move the alias a
  binding follows, and read what the tool actually cost.
