...
async function ensurePrompt(toolIds: string[]): Promise<void> {
  ...
  if (versions.total === 0) {
    // Attach the catalog tools to this immutable version. hub.prompts.render then
    // returns them alongside the messages. First commit auto-creates the
    // `production` alias hub.prompts.render reads below.
...
async function main(): Promise<void> {
  ...
  // hub.prompts.render. `tools` is for tools declared with `acrux.tool`, which carry
...
The flow is store → fetch → use:
  ...
  FETCH + USE (SDK, at run time):
    3. ``hub.prompts.render(name, 'production', vars)`` returns ``(messages, tools)``
       ...
       version's attached tool schemas, already in OpenAI shape. Both come from
       the framework — this file no longer holds them.
    4. ``hub.gateway.run_tool_loop(model, messages, tool_defs=tools, dispatch=dispatch)``
       runs the loop with exactly what was fetched. The only tool code left in this
       file is ``dispatch`` — the local implementations. Schemas, descriptions, and
       the prompt text live on the server.
...
def dispatch(name: str, args: dict[str, Any]) -> Any:
    ...
    if name == "get_weather":
        ...
        return {"city": args.get("city"), "conditions": WEATHER.get(city, "no data for that city")}
...
async def ensure_prompt(tool_ids: list[str]) -> None:
    """Find-or-create the prompt with tools attached."""
    ...
    if versions["total"] == 0:
        await api("POST", f"/prompts/{prompt['id']}/versions", {
            "messages": PROMPT_MESSAGES,
            "tools": [{"toolId": tid} for tid in tool_ids],
        })
        print(f'  + committed v1 for prompt "{PROMPT_NAME}" ({len(tool_ids)} tools attached)')
...
async def main() -> None:
    ...
    async with AcruxCore() as hub:
        ...
        result = await hub.gateway.run_tool_loop(
            MODEL, messages, tool_defs=tools, dispatch=dispatch,
        )
        print(f"\nAssistant: {result.content}")
