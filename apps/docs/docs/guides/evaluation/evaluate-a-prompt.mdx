...
Your app can report feedback too, so a thumbs-down in your own UI lands here without
anyone opening the dashboard. The trace id comes back from the call you are rating:
the SDK returns it as `result.gateway.traceId` (`result.trace_id` from a tool loop),
and a raw HTTP call reads it from the `x-gateway-trace-id` response header — see
[chat, stream, and collect feedback with the SDK](./use-the-sdk-for-chat-and-feedback)
for the full round trip. `source` records who the rating came from — `end_user` for
your app's users, `developer` for your own team; it defaults to `user`.
...
```bash
curl -X POST "$ACRUXCORE_BASE_URL/traces/<trace-id>/feedback" \
  -H "Authorization: Bearer $ACRUXCORE_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"rating": -1, "comment": "Too formal — be warmer", "source": "end_user"}'
```
...
```json
{
  ...
  "source": "end_user",
  "createdBy": "<user-id>",
  "author": { "id": "<user-id>", "name": "Demo", "email": "demo@acruxcore.com" },
  "createdAt": "2026-09-14T07:34:31.627Z",
  "updatedAt": "2026-09-14T07:34:31.627Z"
}
```
...
feedback = requests.post(f"{base}/traces/<trace-id>/feedback", headers=h, json={
    "rating": -1,
    "comment": "Too formal — be warmer",
    "source": "end_user",
}).json()
...
:::tip[No feedback yet?]
A brand-new workspace has no traffic to collect feedback on, so start from the other
end: **New dataset** on that page creates an empty one, and **Add example** on the
dataset writes a row by hand — the prompt variables to render, plus the criteria a
judge should check. The rest of this guide works the same either way.
:::
...
here, the run still starts — AcruxCore just adds a warning to the response and the run page
