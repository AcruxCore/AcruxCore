...
route. `GET` routes are open to any member. `POST /experiments`,
`DELETE /experiments/:id`, `POST /experiments/:id/runs` and `DELETE /runs/:id`
additionally require `requireRole('owner', 'admin', 'editor')` — starting a run
enqueues one paid provider call per (version × model × example) cell, so it is
not a read-only action. A **team-scoped** API key holds no role and is refused
on those four; use a personal key from an editor account. Team-scoped
throughout.
...
A `prompt_id` that is not one of your team's prompts (status 404) — the same answer
whether it belongs to another team or to nobody, so the endpoint never confirms that
an id names a real prompt somewhere:

```bash
curl -X POST $ACRUXCORE_BASE_URL/experiments \
  -H "Authorization: Bearer $ACRUXCORE_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"dataset_id": "<your dataset>", "prompt_id": "<another team's prompt>", "version_ids": ["<a version>"], "models": ["gpt-4o-mini"]}'
```
...
A `version_ids` entry outside your team is the same, and every entry is checked — a
version of your own prompt in the same array does not carry a foreign one through:
...
`errorMessage` carries why a cell produced nothing, and is `null` on every cell
that ran. It is not the same field as `reason`: `reason` is the judge's verdict,
and stays `null` when there was no output to judge. A failed cell has `output:
null`, `traceId: null` and no judge fields at all, so `errorMessage` is the only
account of what happened.

Curled against a run started against a model the team had not registered:

```bash
curl -H "Authorization: Bearer $ACRUXCORE_API_KEY" "$ACRUXCORE_BASE_URL/runs/e9a53392-bf87-47ee-90f9-361e65f19cbc/cells/v1%7Cgpt-4o-mini"
```

Response (status 200) — excerpted to one of its two examples (the other is the
same shape):
...
```json
{
  ...
  "examples": [
    {
      "exampleId": "3ef6fb9a-3cce-4809-a5f8-b0136f34b738",
...
The same run's own row says it too — `error` on `GET /runs/:id` names the shared
reason once when every cell failed the same way, and the first one when they
differ:

```bash
curl -H "Authorization: Bearer $ACRUXCORE_API_KEY" "$ACRUXCORE_BASE_URL/runs/e9a53392-bf87-47ee-90f9-361e65f19cbc"
```

Response (status 200), excerpted to the three fields this section is about —
`id`, `experimentId`, `grid`, `exampleCount` and the timestamps are unchanged
and shown under `GET /api/v1/runs/:id` above:

```json
{
  "status": "failed",
  "error": "Every cell failed. Model 'gpt-4o-mini' is not registered. Add it under Gateway → Models.",
  "results": { "total": 2, "succeeded": 0, "errored": 2 }
}
```
...
Deletes an experiment and **every run under it**, including each run's cells
and reports. The dataset it swept and any prompt version promoted out of one
of its runs are separate resources and are untouched; so is any optimizer
candidate the runs drafted (the candidate's run link is simply cleared).

Refused while any of the experiment's runs is still `queued` or `running` —
the worker is mid-flight and would keep writing results for a row that no
longer exists. Wait for the run to settle, or delete the settled runs
individually with `DELETE /api/v1/runs/:id`.
...
```typescript
await hub.experiments.delete('ff661325-588f-4984-8825-6102acfe3161');
```
...
Already deleted, nonexistent, or another team's id (status 404 — never
confirms whether it exists elsewhere):
...
Deletes one run and every cell it produced, so its report goes with it. The
parent experiment stays — a run is one execution of it — and so does any
optimizer candidate the run drafted, which matters because a candidate may
already have been promoted to a real prompt version.

Refused while the run is still `queued` or `running`, for the same reason as
the experiment delete above.
...
```typescript
await hub.runs.delete('f595cade-4145-49a9-b228-aa344dad15c8');
```
