...
[`versions.mdx`](./versions.mdx) runs before anything is written: transform JS is
...
```typescript
// When using the @acrux.tool decorator, sync is called automatically.
// For raw sync:
const result = await hub.tools.sync([myTool]);
```
...
# When using the @acrux.tool decorator, sync is called automatically.
# For raw sync:
results = await hub.tools.sync([my_tool])
```
