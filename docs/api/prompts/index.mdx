...
2. **`POST /prompts/:id/versions`** — commit a version with the actual `model` and
   `messages`. The first version auto-creates `production` and `staging` aliases
   pointing at v1.
...
Tools are not part of a version. A version holds the template; which tools the
prompt calls is a separate, live setting per prompt alias — see
[Prompt Tool Bindings](/api-reference/prompts/tool-bindings).
...
curl -X POST $ACRUXCORE_BASE_URL/prompts \
  ...
  -d '{"name":"customer-support","description":"Handles customer support queries"}'
```
...
```typescript
const result = await hub.prompts.list();
// with search
const filtered = await hub.prompts.list({ search: 'customer' });
```
...
```python
result = await hub.prompts.list()
# with search
filtered = await hub.prompts.list(search='customer')
```
...
```typescript
const prompt = await hub.prompts.get('5fd454e0-bdb2-4468-b369-7fbb3f252dca');
```
...
```python
prompt = await hub.prompts.get('5fd454e0-bdb2-4468-b369-7fbb3f252dca')
```
...
```bash
curl -X PATCH $ACRUXCORE_BASE_URL/prompts/5fd454e0-bdb2-4468-b369-7fbb3f252dca \
  -H "Authorization: Bearer $ACRUXCORE_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"name":"customer-support-v2"}'
```
...
```typescript
const updated = await hub.prompts.update('5fd454e0-bdb2-4468-b369-7fbb3f252dca', {
...
updated = await hub.prompts.update(
    '5fd454e0-bdb2-4468-b369-7fbb3f252dca',
    name='customer-support-v2',
)
```
...
```bash
curl -X DELETE $ACRUXCORE_BASE_URL/prompts/5fd454e0-bdb2-4468-b369-7fbb3f252dca \
  -H "Authorization: Bearer $ACRUXCORE_API_KEY"
```
...
```typescript
await hub.prompts.delete('5fd454e0-bdb2-4468-b369-7fbb3f252dca');
```
