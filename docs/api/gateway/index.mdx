...
or 422 describes *your request*, so the provider's message is passed through. A 401,
403, 429 or any 5xx describes the connection instead — its credential, its quota, or
the provider's own internals — none of which the caller of this gateway can act on, so
those are summarised:
