...
Only one budget may exist per `(team, scope, period)` — a duplicate returns 409. A
`virtualKeyId` outside your team is a 404, not a 403.
...
A `virtualKeyId` that is not one of your team's keys returns 404 — the same answer
whether the key belongs to another team or to nobody at all:

```bash
curl -X POST $ACRUXCORE_BASE_URL/gateway/budgets \
  -H "Content-Type: application/json" \
  --cookie "connect.sid=<session>" \
  -d '{"period":"month","limitUsd":50,"virtualKeyId":"11111111-2222-3333-4444-555555555555"}'
```

Response (status 404):

```json
{ "error": { "code": "NOT_FOUND", "message": "Virtual key not found." } }
```
