...
(see docs/api/traces/traces.mdx) sharing the same `sessionId`; `traceCount`, `totalCostUsd`
...
```ts
const session = await hub.sessions.get('alpha-session-1');
```
...
```python
session = hub.sessions.get('alpha-session-1')
```
