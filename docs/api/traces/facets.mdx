---
title: "Trace Facets"
description: "Discover the distinct tags, metadata keys/values, resolved span models and declared error codes a team has actually used, for populating filter pickers."
---
...
Returns every distinct tag, every distinct metadata key, every distinct
**resolved** model seen on the team's `llm` spans, and every distinct error code
the team's tools have declared. No query params. Returns empty arrays when the
team has no traces (or none carry tags/metadata/model/error code).
...
`errorCodes` holds the slugs your own tools passed to `ToolResult.error(...)`, and
feeds the `error_code` filter on `GET /api/v1/traces`. There is no facet for
`error_type`: its six values are fixed, so a client can hold them. Error codes are
your team's own words, so the only way to list them is to read back what has been
recorded.
...
```json
{
  ...
  "errorCodes": ["ambiguous_city", "location_not_found"]
}
```

Team-scoped: another team's tags, keys or error codes never appear in the
response. Every list is deduped, sorted alphabetically and capped at 200 values.
