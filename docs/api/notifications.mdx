...
| `connection_health` | A provider connection the gateway cannot call at all, even when a fallback answered. Owners/admins only. |
| `eval_runs` | A run you started finishing or failing. |
| `eval_rules` | A live-traffic rule flagging a low score, or being disabled. Owners/admins only. |
...
curl -X PATCH http://localhost:3001/api/v1/notifications/preferences \
  ...
  -d '{"category":"connection_health","enabled":false}'
```
...
# Response (status 400)
{
  "error": {
    ...
    "message": "Invalid enum value. Expected 'budget_alerts' | 'connection_health' | 'eval_runs' | 'eval_rules' | 'membership' | 'weekly_digest', received 'nope'"
  }
}
```
...
enumerate which users belong to which teams. A mail client's `POST` also always
returns `204` because those clients show the recipient an error on any non-2xx —
and a token we mis-signed is not something they can fix.
:::

:::note[Only POST changes anything]
The `GET` renders a confirmation page and writes nothing. A link in an email is
fetched by things that are not the recipient — security scanners, mail gateways,
clients that prefetch — and none of them clicked. The page's button POSTs the
same token, and that is what applies the opt-out. This is the reason RFC 8058
specifies POST for one-click in the first place.
:::

### POST /api/v1/email/unsubscribe

The one-click target, and the only endpoint here that writes. Idempotent —
posting twice leaves one preference row.

The response depends on who is asking, which the `Accept` header tells apart: a
mail client (no `Accept`, or `*/*`) gets `204`; a browser submitting the
confirmation form (`Accept: text/html`) gets the "Unsubscribed" page, because a
204 would leave the page unchanged and look like the button did nothing.
...
For the many clients that render `List-Unsubscribe` as a plain link. Returns a
minimal, self-contained page — no assets, no tracking, `noindex` — naming the
category and offering one button. **It writes nothing**; the button POSTs the
same token back.
...
```html
<!-- Response (status 200) -->
<!doctype html><html><head><meta charset="utf-8" />…<title>Unsubscribe · acruxcore</title></head>
<body …><div …><h1 …>Unsubscribe</h1>
<p …>Confirm that you no longer want to receive budget alerts for this team.</p>
<form method="post" action="/api/v1/email/unsubscribe?token=<same token>">
<button type="submit" …>Unsubscribe</button></form>
<p …>Nothing has changed yet. Other notification types are unaffected.</p>
</div></body></html>
```

A `GET` with a token that is not ours renders the same page with the generic
wording "these emails", so the endpoint is still not an oracle.
